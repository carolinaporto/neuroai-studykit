...
Write everything in English: the `prompt`, `reference_answer`, `rubric[].point`,
`choices.options`, and any `proposed_topics`.
...
When there is real content, mix types across `free_recall`, `term_def`, `cloze` and `mcq` —
not four questions in the same format:
...
- `cloze`: a single sentence from the chunk (paraphrased or verbatim) with one fact blanked
  out, written as exactly five underscores `_____` in the `prompt`. Exactly one blank per
  item — never more, never zero. `reference_answer` is only the word or short phrase that
  fills it, nothing else.
- `mcq`: a question with `choices.options` (3-5 short strings). Distractors must be
  plausible to someone who half-remembers the material — wrong facts from elsewhere in the
  chunk, not absurd or obviously-wrong options. `reference_answer` must be the exact text of
  the correct option, character-for-character identical to one entry in `choices.options`.

**Rubric length depends on type.** `free_recall` and `term_def` get 2 to 5 rubric points
(rules below). `cloze` and `mcq` get **exactly 1** rubric point — grading for these two
types is a plain text match against `reference_answer`, not rubric coverage, so there is
only one thing to anchor: the fact the blank or the correct option states. That one point
still needs a real `support_quote`, same rules as any other point.

For every item, each rubric point needs:
...
- `weight`: how important the point is to a correct answer, any positive number (points do
  not need to sum to 1; only relative weight matters). For `cloze`/`mcq`'s single point,
  any positive number works — it's the only point, so its weight doesn't affect the score.
- `support_quote`: the EXACT sentence or clause from the chunk above that justifies this
  ...
  inventing one (for `cloze`/`mcq`, drop the item instead — one point is required).
...
**This is a rule about the quote, never about how many points you write.** For
`free_recall`/`term_def`, multiple rubric points MUST share the exact same `support_quote`
when one sentence carries more than one separately gradable idea — one point per idea, each
with its own weight, even if that means copying the identical `support_quote` string into
two or more points.
...
one point worth double. (This doesn't apply to `cloze`/`mcq`, which always have exactly one
point.)

## Hard constraints

- Never let the wording of the question give away the answer: no `support_quote` text, and
  no close paraphrase of one, may appear inside the item's own `prompt`. For `cloze`, the
  blanked fact itself must not appear anywhere else in the prompt.
...
- `reference_answer` is a model answer, in full sentences, that would score every rubric
  point — except for `cloze` (just the blanked word/phrase) and `mcq` (just the correct
  option's exact text).
...
- `choices` is present if and only if `type` is `mcq` — omit it entirely for every other type.
...

{
  "items": [
    {
      "type": "free_recall" | "term_def" | "cloze" | "mcq",
      "prompt": "string",
      "reference_answer": "string",
      "rubric": [
        {"point": "string", "weight": 1.0, "support_quote": "string"}
      ],
      "choices": {"options": ["string", "string", "string"]},
      "difficulty": 1,
...
`choices` only appears on an `mcq` item — leave the whole key out for every other type.
