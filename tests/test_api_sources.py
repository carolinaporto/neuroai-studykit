...
async def _test_client(session_factory: async_sessionmaker) -> AsyncIterator[httpx.AsyncClient]:
    """Overrides `require_owner`/`get_current_user_id` too: this file tests the
    sources-grouping logic, not auth (that's tests/test_auth.py's job) — see
    test_sources_requires_a_signed_in_session there for the 401 behavior itself."""
